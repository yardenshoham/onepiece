	...
	return Layout(title, "/", 7200, analyticsConfig,
		html.Div(g.Attr("class", "dashboard-layout"),
			html.Div(g.Attr("class", "grid"),
				...
				components.Card("⏱ Time Spent Watching",
					watchTimeCard(d),
				),
					...
					g.If(d.AvgEpisodesPerDay > 0,
						g.Group([]g.Node{
							html.P(
								g.Text("~"),
								g.El("relative-time",
									g.Attr("datetime", d.EstimatedCatchUpDate.Format("2006-01-02T15:04:05Z07:00")),
									g.Attr("format", "datetime"),
									g.Text(d.EstimatedCatchUpDate.Format("January 2006")),
								),
							),
				...
				components.Card("📍 Last Watched",
					...
					html.P(
						g.Text("Watched: "),
						g.El("relative-time",
							g.Attr("datetime", d.LastEpisode.WatchedAt.Format("2006-01-02T15:04:05Z07:00")),
							g.Attr("format", "relative"),
							g.Text(d.LastEpisode.WatchedAt.Format("Jan 2, 2006")),
						),
					),
				),
			),
		...
		cards = append(cards, html.Article(
			...
			html.Div(g.Attr("class", "ep-card__body"),
				...
				html.Div(g.Attr("class", "ep-card__footer"),
					html.Span(
						g.El("relative-time",
							g.Attr("datetime", ep.WatchedAt.Format("2006-01-02T15:04:05Z07:00")),
							g.Attr("format", "relative"),
							g.Text(ep.WatchedAt.Format("Jan 2, 2006")),
						),
					),
